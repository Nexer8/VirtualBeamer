        ...
        queueFlushTimer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if(!slidesQueue.isEmpty())
                {
                    ...
                    {
                        if(getSlideSession(tempSlidesQueue.get(i)) == getSlideSession(tempSlidesQueue.get(i+1)) && getSlideSlice(tempSlidesQueue.get(i+1))-getSlideSlice(tempSlidesQueue.get(i)) > 1)
                        {
                            for(int j=1; j < getSlideSlice(tempSlidesQueue.get(i+1))-getSlideSlice(tempSlidesQueue.get(i));j++)
                            {
                                try {
                                    ...
                                    MainService.getInstance().sendPacketLostMessage(InetAddress.getByName(MainService.getInstance().getGroupSession().getLeaderIPAddress()), new Message(MessageType.SLIDE_RESEND,getSlideSession(tempSlidesQueue.get(i)), (short)(getSlideSlice(tempSlidesQueue.get(i))+j)));
                    ...
                    {
                        try {
                            System.out.println("Handling slide ID "+getSlideSession(tempSlidesQueue.get(i)) + " "+ getSlideSlice(tempSlidesQueue.get(i)));
                ...
                if(!messagesQueue.isEmpty())
                {
                        ...
                        if(tempMessageQueue.get(i+1).packetID - tempMessageQueue.get(i).packetID > 1)
                        {
                            for(int j=1; j < tempMessageQueue.get(i+1).packetID - tempMessageQueue.get(i).packetID;j++)
                            {
                                try {
                                    ...
                                    MainService.getInstance().sendPacketLostMessage(InetAddress.getByName(MainService.getInstance().getGroupSession().getLeaderIPAddress()), new Message(MessageType.MESSAGE_RESEND, tempMessageQueue.get(i).packetID+j));
                                    ServerSocket serverSocket = new ServerSocket(PACKET_LOSS_PORT);
                                    Socket socket = serverSocket.accept();
                                    ObjectInputStream in = new ObjectInputStream(socket.getInputStream());
                    ...
                    for(int i=0; i < Math.min(15,tempMessageQueue.size());i++)
                    {
                        try {
                            ...
                            handleMessage(tempMessageQueue.get(i),InetAddress.getByName(MainService.getInstance().getGroupSession().getLeaderIPAddress()));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
